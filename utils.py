...
def estimate_pi(numPoints, rng):
    ...
    Parameters:
    numPoints(int):The number of points to be generated
...
def estimate_pi_sobol(samples):
   ...
   Returns:
   pi_hat: Pi estimate
   points(np.darray): The points generated by the qmc sampler
   """
...
def visualize_pi(n_points,pi,x,y):
    """
...
def visualize_pi_sobol(pi,points):
    ...
    Parameter:
    pi(float): Estimate
    points(int): Number of points to be generated
...
def estimation(selected_points,rng):
    ...
    estimate,x,y = estimate_pi(selected_points,rng)
    err = abs(estimate-pi)
    ...
    with col1:
        ...
        fig = visualize_pi(selected_points,estimate,x,y)
        st.pyplot(fig)
...
def histogram2(rng,sampler,num_points):
    """
   ...
   #Create 4D matrix to host points for each trial in 2D array
    shape=(1,num_trials,num_points,2)
    total_points = num_trials*num_points
    ...
    mc_est,_ = estimate_pi_1000(mc_point,num_points)
    pi_est = mc_est.ravel()

    #Call estimate_pi for qmc methods
    qmc_est,_ = estimate_pi_1000(qmc_point,num_points)
    pi_qmc = qmc_est.ravel()


    #Plot histogram
    ax.hist(pi_est,bins=10,alpha=0.6,density=True,color='tomato',edgecolor='black',label=f'{num_points} points(MC)')
    ax.hist(pi_qmc,bins=10,alpha=0.6,density=True,color='dodgerblue',edgecolor='black',label=f'{num_points} points(QMC)')

    #Customize figure
    ax.axvline(x=np.pi,color='black',linestyle='--',linewidth=1.5,label='Pi')
    ax.legend()
    ax.set_title(f"Comparison of Estimates: N={num_points}; Number of Sims=1000")
    return fig
